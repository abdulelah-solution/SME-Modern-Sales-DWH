...
SQL Server, and orchestrating the Raw layer ingestion pipeline.
"""

import pandas as pd
from typing import Optional
from python.config import BASE_RAW_DATA, logging, engine

logger = logging.getLogger(__name__)

# --- Source Data Assets Configuration ---
# Defining the scope of raw datasets to be ingested into the Bronze layer.
# We distinguish between two primary source systems: 
# 1. CRM: Clean-named files (Customer, Product, Sales).
# 2. ERP: Legacy-coded files (Customers, Locations, Categories).

csv_files = [

    # [Source System: CRM] 
    # Purpose: Provides customer-facing data and sales transactions.
    'cust_info.csv',        # Customer demographics and contact info
    'prd_info.csv',         # Product master data
    'sales_details.csv',    # Transactional sales records

    # [Source System: ERP] 
    # Purpose: Enterprise Resource Planning data with legacy naming conventions.
    'CUST_AZ12.csv',        # ERP-specific customer master (Legacy)
    'LOC_A101.csv',         # Branch and Location mapping
    'PX_CAT_G1V2.csv'       # Product category hierarchy and groupings
...
def fetching_data(filename: str) -> Optional[pd.DataFrame]:
    ...
    """
    file_path = BASE_RAW_DATA / filename.strip()
...
def loading_df_to_sqlserver(table_name: str, df: pd.DataFrame, schema: str = 'raw', engine = engine) -> None:
    """
    ...
    Args:
        ...
        schema (str): Target database schema (defaults to 'raw').
...
def fetching_and_loading_process(files: list[str]) -> None:
    """
    ...
    for file in files:
        ...
        if df is not None and not df.empty:
            loading_df_to_sqlserver(table_name, df)